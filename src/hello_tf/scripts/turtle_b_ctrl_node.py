...
from tf.transformations import quaternion_from_euler    # 四元素转欧拉角
...
if __name__ == '__main__':
    ...
    spawn_request.x = 1
    spawn_request.y = 1
    spawn_request.name = 'robot2'
    spawn_request.theta = radians(90)
    spawn_client.call(spawn_request)
    spawn_client.close()

    # 测试让小乌龟Ｂ运动 geometry_msgs/Twist
    publisher = rospy.Publisher('/robot2/cmd_vel', Twist, queue_size=10)

    # 订阅小乌龟的位置和姿态
    rospy.Subscriber('/robot2/pose', Pose, pose_callback)
